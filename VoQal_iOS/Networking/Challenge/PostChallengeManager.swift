...
struct PostChallengeManager {
    ...
    func postChallenge( thumbnail: UIImage, songTitle: String, singer: String, fileURL: URL, completion: @escaping (postChallengeModel?) -> Void) {
        ...
        guard let jsonData = encodeToJSONData(postChallengeParameter) else {
            completion(nil)
            return
        }
